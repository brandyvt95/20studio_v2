<template>
  <section :class="s.GridImageSlider">
    <div :class="s.titleProject">
      <h2 :class="s.nameProject">{{ content.title }}</h2>
      <p :class="s.infoProject">{{ content.paragraph }}</p>
    </div>

    <div :class="s.container" ref="wrapper">
      <div :class="s.row" ref="listTop">
        <div v-for="(item, index) in content.images.slice(0, 3)" :key="index" :class="s.media">
          <img alt="" :src="item.url" />
        </div>
      </div>
      <div :class="s.row" ref="listBot">
        <div v-for="(item, index) in content.images.slice(3, 6)" :key="index" :class="s.media">
          <img alt="" :src="item.url" />
        </div>
      </div>
    </div>
